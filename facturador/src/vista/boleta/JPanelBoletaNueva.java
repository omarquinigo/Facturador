...
public class JPanelBoletaNueva extends javax.swing.JPanel {
    ...
    private void cargarIdBoleta(){
        String nombreBoleta = "";
        //cargando la serie
        try {
            rs = Config.Consulta("select serieBoleta \n"
                    + "from config;");
            if(rs.last() == true){
    ...
    private void registrarBoleta() {
        ...
        try {
            Boleta.registrarBoleta(id, idCliente, fecha, horaEmision, fechaVencimiento,
    ...
    private void registrarBoletaDet() {
        try {
            ...
            for (int i = 0; i < jtblDetalle.getRowCount(); i++) {
                ...
                String IdBoleta = id;
                Boleta.registrarBoletaDet(Id, IdBoleta, item, cantidad, tipoUnidad,
    ...
    private void initComponents() {
        ...
        jbtnNuevoComprobante.setText("Nueva Boleta");
    ...
    private void jbtnCrearArchivosPlanosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnCrearArchivosPlanosActionPerformed
        if (validar().equals("")) {
            ...
            if (Boleta.existeBoleta(id) == false) {
