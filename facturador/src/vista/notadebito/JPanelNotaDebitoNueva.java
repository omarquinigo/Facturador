...
public class JPanelNotaDebitoNueva extends javax.swing.JPanel {
    ...
    private void cargarIdNotaDebito(){
        String nombreNotaDebito;

        //cargando la serie
        String serieNDebito;
        if (idComprobante.contains("FF")) {
            serieNDebito = "FD" + Datos.getSerieNDebitoFactura();
        } else {
            serieNDebito = "BD" + Datos.getSerieNDebitoBoleta();
        }

        try {
            rs = NotaDebito.consulta("select * \n"
                    + "from notadebito \n"
                    + "where id LIKE '%" + serieNDebito + "%';");
            if (rs.last() == true) {
                // obtengo el id
                ...
                nombreNotaDebito = serieNDebito + "-" + numeroNotaDebito;
                id = nombreNotaDebito;
                jtxtId.setText(id);
            } else {
                nombreNotaDebito = serieNDebito + "-" + "00000001";
                id = nombreNotaDebito;
                jtxtId.setText(id);
            }
            rs.close();
    ...
    private void cambiarMotivo() {
        ...
        switch (motivo){
            case "-Seleccione-":// seleccione
                ...
                activarBloquearCamposDetalle(false);
                activarBloquearBotonesDetalle(false);
            ...
            case "Intereses por mora":
                ...
                limpiarCamposDetalle();
                cargarComprobante();
                activarBloquearCamposDetalle(true);
                activarBloquearBotonesDetalle(false);
    ...
    private void activarBloquearCamposDetalle(boolean b) {
        jtxtCantidad.setEnabled(b);
        jcbxTipoUnidad.setEnabled(b);
        jcbxPrecioUnitario.setEnabled(b);
        jtxtCodigo.setEnabled(b);
        jtxtDescripcion.setEnabled(b);
        jcbxTributo.setEnabled(b);
    }
    
    private void activarBloquearBotonesDetalle(boolean b) {
        jbtnAgregar.setEnabled(b);
        jbtnQuitar.setEnabled(b);
        jbtnEditar.setEnabled(b);
        jbtnActualizar.setEnabled(b);
    }
    ...
    private void registrarNotaDebitoDet() {
        try {
            ...
            for (int i = 0; i < jtblDetalle.getRowCount(); i++) {
                ...
                String IdBoleta = id;
                NotaDebito.registrarNotaDebitoDet(Id, IdBoleta, item, cantidad, tipoUnidad,
                        codigo, descripcion, tributo, montoTributo, tipoAfectacionTributo,
                        precioUnitario, valorUnitarioGratuito, precioTotal);
            }
        } catch (Exception e) {
            System.out.println("Error registrando detalle de nota de débito en base de datos: \n" + e);
            Metodos.mensajeError("Error registrando detalle de nota de débito en base de datos: \n" + e);            
        }
    }
    
    private void crearArchivosPlanos(){
        boolean validar = Metodos.validarExisteAPNotaDebito(id);
        if (validar == true) {
            Metodos.mensajeError("Uno o varios AP del comprobante " + id + " ya existe.\n"
                    + "Elimínelos manualmente y vuelva a crear los AP.");
            jbtnCrearArchivosPlanos.setEnabled(true);
        } else {
            ArchivosPlanos.apNotaDebito(id);
    ...
    private void initComponents() {
        ...
        jcbxMotivo.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "-Seleccione-", "Intereses por mora", "Penalidades/otros conceptos" }));
    ...
    private void jbtnCrearArchivosPlanosActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnCrearArchivosPlanosActionPerformed
        if (validar().equals("")) {
            if (NotaDebito.existeNotaDebito(id) == false) {
    ...
    private void jbtnNuevoComprobanteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnNuevoComprobanteActionPerformed
        JPanelNotaDebitoNueva jpncn = new JPanelNotaDebitoNueva();
